...
async function main() {
  ...
  if (command !== 'scan') {
    console.error('usage: xpr-arb-radar scan [--min-edge=1] [--min-confidence=indicative] [--json]');
    process.exit(1);
  }
  ...
  if (process.argv.includes('--json')) {
    console.log(JSON.stringify({ scannedAt: new Date().toISOString(), quoteCount: quotes.length, minConfidence, failures, opportunities }, null, 2));
    return;
  }
  ...
  console.log(`quotes: ${quotes.length} | confidence: ${JSON.stringify(confidenceCounts)} | opportunities >= ${minEdge}% net (${minConfidence}+): ${opportunities.length}`);
