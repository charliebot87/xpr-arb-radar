...
async function main() {
  ...
  if (command !== 'scan') {
    console.error('usage: xpr-arb-radar scan [--min-edge=1] [--json]');
    process.exit(1);
  }
  ...
  const opportunities = findOpportunities(quotes, Number.isFinite(minEdge) ? minEdge : 1);

  if (process.argv.includes('--json')) {
    console.log(JSON.stringify({ scannedAt: new Date().toISOString(), quoteCount: quotes.length, failures, opportunities }, null, 2));
    return;
  }

  console.log(`xpr-arb-radar watch-only scan`);
  console.log(`quotes: ${quotes.length} | opportunities >= ${minEdge}% net: ${opportunities.length}`);
  ...
  for (const o of opportunities.slice(0, 20)) {
    ...
    console.log(`  edge gross ${o.grossEdgePct.toFixed(2)}% | net ${o.netEdgePct.toFixed(2)}%`);
